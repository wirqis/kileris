...
RSFirewall.GeoIPDownload = function (element, version) {
	RSFirewall.$(element).attr('disabled', 'disabled');
	
	var GeoIPDownloadErrorHandler = function(message) {
		RSFirewall.$(element).parent().append('<p><strong>' + message + '</p></strong>').hide().fadeIn();
		RSFirewall.$(element).parent().append('<p>' + Joomla.JText._('COM_RSFIREWALL_GEOIP_DB_CANNOT_DOWNLOAD') + '</p><p><input type="file" name="jform[geoip_upload][]"></p><p>' + Joomla.JText._('COM_RSFIREWALL_GEOIP_DB_CANNOT_DOWNLOAD_CONTINUED') + '</p>').hide().fadeIn();
	...
	RSFirewall.$.ajax({
		...
		success: function (data, textStatus, jqXHR) {
			if (!data.success) {
				GeoIPDownloadErrorHandler(data.message);
			} else {
				if (version == 'v4' && !RSFirewall.$('.rsfirewall-geoip-works').length) {
