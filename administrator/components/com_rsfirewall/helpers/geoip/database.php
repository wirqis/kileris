...
define("RSF_MAX_ORG_RECORD_LENGTH", 300);
...
define("RSF_GEOIP_DOMAIN_EDITION", 11);
define("RSF_GEOIP_COUNTRY_EDITION_V6", 12);
define("RSF_GEOIP_LOCATIONA_EDITION", 13);
define("RSF_GEOIP_ACCURACYRADIUS_EDITION", 14);
define("RSF_GEOIP_CITYCOMBINED_EDITION", 15);
define("RSF_GEOIP_CITY_EDITION_REV1_V6", 30);
define("RSF_GEOIP_CITY_EDITION_REV0_V6", 31);
define("RSF_GEOIP_NETSPEED_EDITION_REV1", 32);
define("RSF_GEOIP_NETSPEED_EDITION_REV1_V6", 33);
define("RSF_GEOIP_USERTYPE_EDITION", 28);
define("RSF_GEOIP_USERTYPE_EDITION_V6", 29);
define("RSF_GEOIP_ASNUM_EDITION_V6", 21);
define("RSF_GEOIP_ISP_EDITION_V6", 22);
define("RSF_GEOIP_ORG_EDITION_V6", 23);
define("RSF_GEOIP_DOMAIN_EDITION_V6", 24);

define("RSF_CITYCOMBINED_FIXED_RECORD", 7);

define("RSF_FULL_RECORD_LENGTH", 50);
// Copyright 2016 MaxMind, Inc. All Rights Reserved
global $RSF_GEOIP_REGION_NAME;
$RSF_GEOIP_REGION_NAME = array (
	'AD' =>
		array (
			'02' => 'Canillo',
			'03' => 'Encamp',
	...
	if ($gi->flags & RSF_GEOIP_SHARED_MEMORY)
	{
		...
		for ($i = 0; $i < RSF_STRUCTURE_INFO_MAX_SIZE; $i++)
		{
			...
			if ($delim == (chr(255) . chr(255) . chr(255)))
			{
				...
				elseif ($gi->databaseType == RSF_GEOIP_REGION_EDITION_REV1)
				{
					$gi->databaseSegments = RSF_GEOIP_STATE_BEGIN_REV1;
				}
				elseif (($gi->databaseType == RSF_GEOIP_CITY_EDITION_REV0)
					|| ($gi->databaseType == GEOIP_CITY_EDITION_REV1)
					|| ($gi->databaseType == RSF_GEOIP_ORG_EDITION)
					|| ($gi->databaseType == RSF_GEOIP_ORG_EDITION_V6)
					|| ($gi->databaseType == RSF_GEOIP_DOMAIN_EDITION)
					|| ($gi->databaseType == RSF_GEOIP_DOMAIN_EDITION_V6)
					|| ($gi->databaseType == RSF_GEOIP_ISP_EDITION)
					|| ($gi->databaseType == RSF_GEOIP_ISP_EDITION_V6)
					|| ($gi->databaseType == RSF_GEOIP_USERTYPE_EDITION)
					|| ($gi->databaseType == RSF_GEOIP_USERTYPE_EDITION_V6)
					|| ($gi->databaseType == RSF_GEOIP_LOCATIONA_EDITION)
					|| ($gi->databaseType == RSF_GEOIP_ACCURACYRADIUS_EDITION)
					|| ($gi->databaseType == RSF_GEOIP_CITY_EDITION_REV0_V6)
					|| ($gi->databaseType == RSF_GEOIP_CITY_EDITION_REV1_V6)
					|| ($gi->databaseType == RSF_GEOIP_NETSPEED_EDITION_REV1)
					|| ($gi->databaseType == RSF_GEOIP_NETSPEED_EDITION_REV1_V6)
					|| ($gi->databaseType == RSF_GEOIP_ASNUM_EDITION)
					|| ($gi->databaseType == RSF_GEOIP_ASNUM_EDITION_V6)
				)
				{
		...
		if (($gi->databaseType == RSF_GEOIP_COUNTRY_EDITION) ||
			($gi->databaseType == RSF_GEOIP_COUNTRY_EDITION_V6) ||
...
function rsfirewall_sharedMemRead($gi, $start, $maxLength)
{
...
function rsfirewall_geoip_country_id_by_name_v6($gi, $name)
{
	...
	return rsfirewall_geoip_country_id_by_addr_v6($gi, $addr);
}
...
function rsfirewall_geoip_country_code_by_name_v6($gi, $name)
{
...
function rsfirewall_geoip_country_id_by_addr_v6($gi, $addr)
{
	$ipnum = inet_pton($addr);

	return rsfirewall_geoip_seek_country_v6($gi, $ipnum) - RSF_GEOIP_COUNTRY_BEGIN;
}
...
function rsfirewall_geoip_country_code_by_addr_v6($gi, $addr)
{
	$country_id = rsfirewall_geoip_country_id_by_addr_v6($gi, $addr);
	if ($country_id !== false)
	{
		return $gi->GEOIP_COUNTRY_CODES[$country_id];
	}
...
function rsfirewall_geoip_country_name_by_addr_v6($gi, $addr)
{
	$country_id = rsfirewall_geoip_country_id_by_addr_v6($gi, $addr);
	if ($country_id !== false)
	{
...
function rsfirewall_geoip_seek_country_v6($gi, $ipnum)
{
	...
	for ($depth = 127; $depth >= 0; --$depth)
	{
		...
		{
			$buf = rsfirewall_sharedMemRead($gi,
				2 * $gi->record_length * $offset,
				2 * $gi->record_length
			);
		}
...
function rsfirewall_common_get_org($gi, $seek_org)
{
	$record_pointer = $seek_org + (2 * $gi->record_length - 1) * $gi->databaseSegments;
	if ($gi->flags & RSF_GEOIP_SHARED_MEMORY)
	{
		$org_buf = rsfirewall_sharedMemRead($gi, $record_pointer, RSF_MAX_ORG_RECORD_LENGTH);
	}
	else
	{
		fseek($gi->filehandle, $record_pointer, SEEK_SET);
		$org_buf = fread($gi->filehandle, RSF_MAX_ORG_RECORD_LENGTH);
	}
	$org_buf = rsfirewall_safe_substr($org_buf, 0, strpos($org_buf, "\0"));

	return $org_buf;
}

function rsfirewall_get_org_v6($gi, $ipnum)
{
	$seek_org = rsfirewall_geoip_seek_country_v6($gi, $ipnum);
	if ($seek_org == $gi->databaseSegments)
	{
		return null;
	}

	return rsfirewall_common_get_org($gi, $seek_org);
}
...
function rsfirewall_geoip_name_by_addr_v6($gi, $addr)
{
	if ($addr == null)
	{
		return 0;
	...
	return rsfirewall_get_org_v6($gi, $ipnum);
}

function rsfirewall_geoip_name_by_addr($gi, $addr)
{
	if ($addr == null)
	{
		return 0;
...
{
	return rsfirewall_geoip_name_by_addr($gi, $addr);
}
...
function rsfirewall_get_region($gi, $ipnum)
{
	...
	elseif ($gi->databaseType == RSF_GEOIP_REGION_EDITION_REV1)
	{
		...
		{
			$country_code = $gi->GEOIP_COUNTRY_CODES[(int) (($seek_region - RSF_WORLD_OFFSET) / RSF_FIPS_RANGE)];
			$region       = "";
		}
...
function rsfirewall_safe_substr($string, $start, $length)
{
...
function rsfirewall_get_record_v6($gi, $ipnum)
{
	$seek_country = rsfirewall_geoip_seek_country_v6($gi, $ipnum);
	if ($seek_country == $gi->databaseSegments) {
		return null;
	}
	return rsfirewall_common_get_record($gi, $seek_country);
}

function rsfirewall_common_get_record($gi, $seek_country)
{
	// workaround php's broken substr, strpos, etc handling with
	// mbstring.func_overload and mbstring.internal_encoding
	$mbExists = extension_loaded('mbstring');
	...
	if ($gi->flags & RSF_GEOIP_MEMORY_CACHE) {
		$record_buf = substr($gi->memory_buffer, $record_pointer, RSF_FULL_RECORD_LENGTH);
	} elseif ($gi->flags & RSF_GEOIP_SHARED_MEMORY) {
		$record_buf = rsfirewall_sharedMemRead($gi, $record_pointer, RSF_FULL_RECORD_LENGTH);
	} else {
		fseek($gi->filehandle, $record_pointer, SEEK_SET);
		$record_buf = fread($gi->filehandle, RSF_FULL_RECORD_LENGTH);
	}
	$record = new rsfirewall_geoiprecord;
	$record_buf_pos = 0;
...
function rsfirewall_geoip_record_by_addr_v6($gi, $addr)
{
	if ($addr == null) {
		return 0;
	}
	$ipnum = inet_pton($addr);
	return rsfirewall_get_record_v6($gi, $ipnum);
}

function rsfirewall_get_record($gi, $ipnum)
{
	$seek_country = rsfirewall_geoip_seek_country($gi, $ipnum);
	if ($seek_country == $gi->databaseSegments) {
		return null;
	}
	return rsfirewall_common_get_record($gi, $seek_country);
...
function rsfirewall_geoip_record_by_addr($gi, $addr)
{
	if ($addr == null) {
		return 0;
	}
	$ipnum = ip2long($addr);
	return rsfirewall_get_record($gi, $ipnum);
}
